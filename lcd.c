...
{
    // LCD_SCK
    LCD_SCK_PORT->SEL0 |= LCD_SCK_PIN;  //want to select the primary function
    LCD_SCK_PORT->SEL1 &= ~LCD_SCK_PIN;
    // LCD_MOSI
    LCD_MOSI_PORT->SEL0 |= LCD_MOSI_PIN; //select the primary function
    LCD_MOSI_PORT->SEL1 &= ~LCD_MOSI_PIN;
    // LCD_CS
    LCD_CS_PORT->DIR |= LCD_CS_PIN; //set as output
    // LCD_RST
    LCD_RST_PORT->DIR |= LCD_RST_PIN; //set as output
    // LCD_RS
    LCD_DC_PORT->DIR |= LCD_DC_PIN; //set as output
}
...
void HAL_LCD_SpiInit(void)
{
    ...
    EUSCI_B0->CTLW0 = EUSCI_B_CTLW0_CKPH | EUSCI_B_CTLW0_MSB | EUSCI_B_CTLW0_MST
            ...
            | EUSCI_B_CTLW0_SSEL__SMCLK | EUSCI_B_CTLW0_SWRST;
